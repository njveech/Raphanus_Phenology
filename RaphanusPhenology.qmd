...
Specific procedures and approaches were developed after discussing the logistics and potential inconsistencies that could occur when training a Machine Learning model to recognize the three different reproductive classes: bud clusters, flowers, and fruit. Bud clusters fall along a spectrum of appearances. Some bud
...
```{r}
#Read in full complete data CSV, and add col with month abbreviations in text
...
Notes: Main takeaway: it seems odd that there would only be 1 bud cluster. This is not reliable.
...
-Write out logistics to make them simpler if breaking apart thinking into very specific steps in plain English.
...
Notes: don't really need to do this for fruits if do for flowers
...
ggplot(flowering, aes(x = month)) + geom_histogram(binwidth = 1, fill = "firebrick3", color = "black") + scale_x_continuous(breaks = 1:12, labels = month.abb) + labs(x = "Month", y = "Frequency", title = "Frequency of Flowering Specimens Collected in each Month ") + theme(panel.grid.minor.x = element_blank())
```
...
hist(fulldata$year, breaks=60)

# make vector with ids of sierran samples
sierra_ids <- c("1311269", "1312145", "1777873", "3737681", "2289763", "2342720", "2345323", "2358602", "2359565", "2370292", "2433969", "2442814", "4488222")

# filter out all sierrans
fulldata_no.sierra <- fulldata %>% filter(!id %in% sierra_ids)
...
fulldata_so_cal <- fulldata %>% filter(county %in% c("Santa Barbara", "San Diego",)


old <- fulldata_so_cal %>% filter(year < 1950 ) %>% filter(Flower > 0)

hist(old$month, breaks = seq(0, 12, 1), freq = TRUE, col = "firebrick3",
     border = "white", main = "Frequency of Specimens Collected While Flowering
    pre-1950", xlab = "Month", ylab = "Frequency")


recent <- fulldata_so_cal %>% filter(year > 2000) %>% filter(Flower > 0) 

hist(recent$month, breaks = seq(0, 12, 1), freq = TRUE, col = "firebrick3",
     border = "white", main = "Frequency of Specimens Collected While Flowering
   post-2000", xlab = "Month", ylab = "Frequency")
