...
TASK FOR NATALIE: In the chunk below, it now reads in the new, full climate dataset. there are additional variables from before and from what is named in the second 'line' (the "rearrange columns" line). Read in the new dataset to generate all_data. Then, use colnames() to get a list of the varibles. Determine ehat each acronym stands for (hopefully you can find them on the page linked above), and add to the last line of the code that reads in and reformats the data to rename them to more intuitive things (or at least get rid of the weird tailing "_"). Then re-run the whole chunk to generate the new climateNA_full_data_tall.csv. Open that in excel to check and make sure it looks good!
